...
import VerifyBillingEntryButton from "./VerifyBillingEntryButton";
...
const BillingEntriesTable = ({
  ...
  const columns: ColumnsType<BillingEntryDTO> = [
    ...
    },
    {
      ...
      render: (text: string, record) => {
        if (record.billId != null) {
          return <Tag color="blue">HĐ-#{record.billId}</Tag>;
        }
        if (record.isVerified == null) {
          return <Tag color="orange">Chờ xác nhận</Tag>;
        } else if (record.isVerified == false) {
          return <Tooltip title={record.reason}></Tooltip>;
        } else {
          return <Tag color="yellow">Chưa có hóa đơn</Tag>;
        }
      },
    },
    {
    ...
    },
    {
      ...
      render: (_, record) => {
        ...
        if (user?.id == tutorLearnerSubjectDetail?.learnerId) {
          return (
            <VerifyBillingEntryButton
              billingEntryId={record.billingEntryID!}
              tutorLearnerSubjectId={record.tutorLearnerSubjectId!}
  ...
  const rowSelection: TableProps<BillingEntryDTO>["rowSelection"] = {
    ...
    getCheckboxProps: (record: BillingEntryDTO) => ({
      ...
      disabled: record.billId !== null || !record.isVerified,
    }),
  };
                ...
                <Table.Summary.Cell index={4} colSpan={2}>
                  {user?.id == tutorLearnerSubjectDetail?.tutorId && (
                    <GenBillButton
                      ...
                      learnerMail={
                        tutorLearnerSubjectDetail?.learnerEmail! || ""
                      }
