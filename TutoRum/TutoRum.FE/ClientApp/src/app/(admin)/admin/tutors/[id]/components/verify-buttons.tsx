...
const reasonSchema = z.object({
  reason: z.string({ required_error: "Lý do từ chối là bắt buộc." }),
});
...
export const VerifyButton = ({
  ...
  >(null);
  const { mutateAsync, isLoading } = useVerifyStatus();
  ...
  const handleUnverify = form.handleSubmit(async (data) => {
    try {
      ...
      toast.success(
        mode === "admin" ? "Từ chối thành công" : "Yêu cầu đã bị từ chối"
      );
      setButtonIsVerified(false);
      setIsModalVisible(false);
    } catch (error) {
      toast.error(
        mode === "admin" ? "Từ chối thất bại" : "Không thể từ chối yêu cầu"
      );
