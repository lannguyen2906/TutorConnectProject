    ...
    public class AccountsController : ApiControllerBase
    {
        ...
        public async Task<IActionResult> SignUp(SignUpModel signUpModel)
        {
            try
            {
                ...
                if (result.Succeeded)
                {
                    var signedUser = await accountRepo.GetUserByEmailAsync(signUpModel.Email);

                    if (signedUser != null)
                    {
                        var token = await accountRepo.GenerateEmailConfirmationTokenAsync(signedUser);
                        var confirmationLink = Url.Action("ConfirmEmail", "Accounts",
                                        new { userId = signedUser.Id, token = token }, Request.Scheme);
                    var emailContent = $@"
                    <!DOCTYPE html>
                    <html lang='vi'>
                    <head>
                    ...
                    // Gửi email với confirmationLink
                    await _emailService.SendEmailAsync(signedUser.Email, "[TutorConnect] Xác nhận tài khoản của bạn", emailContent);
        ...
        public async Task<IActionResult> SignIn(SignInModel signInModel)
        {
            ...
            if (user == null)
            {
                var unauthorizedResponse = ApiResponseFactory.Unauthorized<object>("Tài khoản không tồn tại");
                return Ok(unauthorizedResponse);
            }

            if (user.EmailConfirmed == false)
            {
                var unauthorizedResponse = ApiResponseFactory.Unauthorized<object>("Tài khoản của bạn chưa xác thực emai");
                return Ok(unauthorizedResponse);
            }
            ...
            {
                var unauthorizedResponse = ApiResponseFactory.Unauthorized<object>("Tài khoản của bạn đã bị block");
                return Ok(unauthorizedResponse);
            }
