...
namespace TutoRum.FE.Controllers
{
    ...
    public class BillingEntryController : ApiControllerBase
    {
        ...
        [HttpPut]
        [Route(Common.Url.User.BillingEntry.VerifyBillingEntry)]
        [ProducesResponseType(typeof(ApiResponse<object>), 200)]
        public async Task<IActionResult> VerifyBillingEntry([FromQuery] int billingEntryId, bool isVerified, string? reason)
        {
            try
            {
                await _billingEntryService.VerifyBillingEntry(billingEntryId, isVerified, reason, User);
                var response = ApiResponseFactory.Success<object>(null);
                return Ok(response);
            }
            catch (UnauthorizedAccessException ex)
