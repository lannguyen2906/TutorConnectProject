...
namespace TutoRum.FE.Controllers
{
    public class AdminController : ApiControllerBase
    {
        ...
        [HttpGet]
        [Route(Common.Url.User.Admin.GetDashboardKeyMetrics)]
        [ProducesResponseType(typeof(ApiResponse<DashboardKeyMetricsDto>), 200)]
        public async Task<IActionResult> GetDashboardKeyMetrics()
        {
            try
            { 

                var tutorsHomePage = await _adminSevice.GetDashboardKeyMetrics(User);
                var response = ApiResponseFactory.Success(tutorsHomePage);
                return Ok(response);
            }
            catch (UnauthorizedAccessException)
        ...
        [HttpGet]
        [Route(Common.Url.User.Admin.GetDashboardChartsData)]
        [ProducesResponseType(typeof(ApiResponse<DashboardChartData>), 200)]
        public async Task<IActionResult> GetDashboardChartsData()
        {
            try
            {
                var tutorsHomePage = await _adminSevice.GetDashboardChartsData(User);
                var response = ApiResponseFactory.Success(tutorsHomePage);
                return Ok(response);
            }
            catch (UnauthorizedAccessException)
