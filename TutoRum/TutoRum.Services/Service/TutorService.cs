...
namespace TutoRum.Services.Service
{
    public class TutorService : ITutorService
    {
        ...
        public async Task<TutorDto> GetTutorByIdAsync(Guid id)
        {
            ...
            var teachingClassrooms = await _unitOfWork.TutorLearnerSubject.GetSubjectsByUserIdAndRoleAsync(id, false, true, true);

            foreach (var tutorSubject in tutorDto.TutorSubjects)
            {

                tutorSubject.NumberOfClass = teachingClassrooms.Count(tc => tc.TutorSubjectId == tutorSubject.TutorSubjectId);
            }
        ...
        public async Task<TutorHomePageDTO> GetTutorHomePage(TutorFilterDto? tutorFilterDto, int index = 0, int size = 20)
        {
            ...
            Expression<Func<Tutor, bool>> filter = tutor =>
                ...
                (!tutorFilterDto.Gender.HasValue || tutor.TutorNavigation.Gender == tutorFilterDto.Gender) &&
            ...
            foreach (var tutorDto in tutorsDto)
            {
                ...
                var teachingClassrooms = await _unitOfWork.TutorLearnerSubject.GetSubjectsByUserIdAndRoleAsync(tutorDto.TutorId, false, true, true);

                tutorDto.NumberOfTeachingHour = teachingClassrooms
                    .Sum(tc => tc.BillingEntries
                         .Sum(be => be.EndDateTime.HasValue && be.StartDateTime.HasValue
                            ? (be.EndDateTime.Value - be.StartDateTime.Value).TotalHours
                            : 0));
