...
namespace TutoRum.Services.Service
{
    public class BillingEntryService : IBillingEntryService
    {
        ...
        public BillingEntryService(IUnitOfWork unitOfWork, UserManager<AspNetUser> userManager, INotificationService notificationService)
        {
        ...
        public async Task AddBillingEntryAsync(AdddBillingEntryDTO billingEntryDTO, ClaimsPrincipal user = null)
        {
            ...
            var tutorLearnerSubject = await _unitOfWork.TutorLearnerSubject.GetTutorLearnerSubjectAsyncById(billingEntryDTO.TutorLearnerSubjectId);

            if (tutorLearnerSubject != null) {
                var notificationDto = new NotificationRequestDto
                {
                    UserId = tutorLearnerSubject.LearnerId ?? new Guid(),
                    Title = "Bạn có 1 buổi học mới được cập nhật của lớp " + tutorLearnerSubject.TutorSubject.Subject.SubjectName,
                    Description = "Bạn vui lòng vào xác nhận buổi học nhé",
                    NotificationType = NotificationType.TutorLearnerSubjectResult,
                    Href = "/user/learning-classrooms/" + tutorLearnerSubject.TutorLearnerSubjectId + "/billing-entries"
                };
        ...
        public async Task VerifyBillingEntry(int billingEntryId, bool isVerified, string? reason, ClaimsPrincipal user)
        {
            ...
            var billingEntry = await _unitOfWork.BillingEntry.FindAsync(b => b.BillingEntryId == billingEntryId);
            ...
            if(isVerified == true)
            {
                billingEntry.IsVerified = true;
            }else
            {
                billingEntry.IsVerified = false;
                billingEntry.reasonDesc = reason;
            }

            _unitOfWork.BillingEntry.Update(billingEntry);
            await _unitOfWork.CommitAsync();
        }
        ...
        public async Task<BillingEntryDTOS> GetAllBillingEntriesAsync(ClaimsPrincipal user, int pageIndex = 0, int pageSize = 20)
        {
            ...
            var billingEntryDTOs = billingEntries.Select(billingEntry => new BillingEntryDTO
            {
                ...
                IsVerified = billingEntry.IsVerified,
                Reason = billingEntry.reasonDesc,
            }).ToList();
